...
import { 
  UserCircle, Dumbbell, History as HistoryIcon, 
  Link as LinkIcon 
} from 'lucide-react';
...
export default function App() {
  ...
  // --- HIER IST DIE REPARIERTE FUNKTION ---
  const handleSaveCustomLog = (title: string, duration: string, note: string) => {
    persistence.saveHistoryEntry({
        id: Date.now(),
        workoutId: -1, // ID -1 signalisiert ein freies Training
        workoutTitle: title,
        date: new Date().toISOString(),
        week: activeWeek,
        type: "Custom",
  ...
  const handleImportPlanAction = (json: any) => {
    if (!json || (!json.data && !json.workouts && !Array.isArray(json))) {
      alert("Ungültiges Format.");
      return;
    }
    persistence.importData(json);
    setShowPastePlanModal(false);
  };
  ...
  return (
    <div className="min-h-screen bg-neutral-900 flex justify-center font-sans">
      <div className="w-full max-w-md bg-gray-50 min-h-screen relative shadow-2xl overflow-hidden flex flex-col">
        ...
        {/* HIER WURDE GEÄNDERT: onSave ruft jetzt handleSaveCustomLog auf */}
        ...
        {showEquipmentModal && (
          <EquipmentModal 
            ...
            equipment={persistence.equipment as any} 
            onSave={(newEq: any) => persistence.updateEquipment(newEq)} 
          />
        )}
        
        {showPastePlanModal && <PastePlanModal onClose={() => setShowPastePlanModal(false)} onImport={handleImportPlanAction} />}
        
        {activePromptModal && (
          <PromptModal
            type={activePromptModal as any}
            value={(persistence.prompts as any)[activePromptModal]}
            onClose={() => setActivePromptModal(null)}
            onSave={(newValue: string) => {
              persistence.updatePrompts(activePromptModal, newValue);
        ...
        {showExitDialog && (
          <ExitDialog 
            ...
            onSave={() => { session.endSession(); setShowExitDialog(false); }} 
            onDiscard={() => { session.endSession(); setShowExitDialog(false); }} 
        ...
        {session.phase !== 'idle' ? (
          <>
            {session.phase === 'warmup' && <WarmupScreen prompt={persistence.prompts.warmup} onComplete={(t: number) => { session.setTotalSeconds(t); session.setPhase('training'); }} onBack={() => setShowExitDialog(true)} />}
            ...
            {session.phase === 'training' && (
              <ActiveWorkoutScreen 
                ...
                isRestActive={session.isRestActive} restSeconds={session.restSeconds} activeRestContext={session.activeRestContext} onSkipRest={session.skipRest}
              />
            )}
          </>
        ) : (
          <div className="pb-24 min-h-screen">
            ...
            {activeTab === 'links' && (
              <LinksScreen 
                links={persistence.links} 
                onAddLink={persistence.addLink} 
                onDeleteLink={persistence.deleteLink} 
              />
            )}
          ...
          <div className="fixed bottom-0 w-full max-w-md mx-auto bg-white border-t border-gray-200 px-2 py-2 pb-6 flex justify-between items-center text-xs font-medium z-20">
            {['profile', 'training', 'links', 'history'].map(tab => (
                <button 
                  ...
                  className={`flex flex-col items-center gap-1 p-2 rounded-lg flex-1 transition-colors ${activeTab === tab ? 'text-blue-800' : 'text-gray-400'}`}
                >
